#!/usr/bin/env bash
# make-icon.sh: renders raycast/assets/icon.png from swap_icon.svg, cropping the
...
rsvg-convert -w 512 -h 512 .tmp/icon.svg -o raycast/assets/icon.png

# reads the IHDR chunk directly, no Pillow or ImageMagick dependency
python3 - raycast/assets/icon.png "$BRAND" <<'PY'
